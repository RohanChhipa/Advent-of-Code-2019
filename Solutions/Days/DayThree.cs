...
//            Console.WriteLine(ProblemOne(inputs));
        ...
        public int ProblemOne(string[] input)
        {
            var wireOneGrid = new int[rows, cols];
            var wireTwoGrid = new int[rows, cols];

            var wireOneDirections = ExtractDirections(input[0]);
            var wireTwoDirections = ExtractDirections(input[1]);

            var wireOnePath = ConstructPath(wireOneDirections);
            var wireTwoPath = ConstructPath(wireTwoDirections);

            PopulateGrid(wireOnePath, wireOneGrid);
            PopulateGrid(wireTwoPath, wireTwoGrid);

            var intersections = new List<(int, int)>();
            for (var k = 0; k < rows; k++)
            for (var j = 0; j < cols; j++)
                if (wireOneGrid[k, j] == 1 && wireTwoGrid[k, j] == 1)
                    intersections.Add((j, k));

            var min = intersections.Select(tuple => Math.Abs(tuple.Item1 - (rows / 2)) + Math.Abs(tuple.Item2 - (rows / 2)))
                .Where(i => i != 0).Min();
        ...
        private void PopulateGrid((int, int)[] wireOneDirections, int[,] wireGrid)
        {
            for (var k = 1; k < wireOneDirections.Length; k++)
            {
                var a = wireOneDirections[k - 1];
                var b = wireOneDirections[k];

                if (IsVertical(a, b))
                {
                    var c = Math.Min(a.Item2, b.Item2);
                    var d = Math.Max(a.Item2, b.Item2);

                    for (var j = c; j <= d; j++)
                        wireGrid[j, a.Item1] = 1;
                }
                else
                {
                    var c = Math.Min(a.Item1, b.Item1);
                    var d = Math.Max(a.Item1, b.Item1);

                    for (var j = c; j <= d; j++)
                        wireGrid[a.Item2, j] = 1;
                }
            }
        }
        ...
        public int ProblemTwo(string[] input)
        {
            var wireOneGrid = new int[rows, cols];
            var wireTwoGrid = new int[rows, cols];
            ...
            intersections = intersections.Where(tuple =>
                (Math.Abs(tuple.Item1 - (rows / 2)) + Math.Abs(tuple.Item2 - (rows / 2))) > 0)
                .ToList();

            var wireOneSteps = intersections.Select(tuple => GetDistance(tuple, wireOnePath));
            var wireTwoSteps = intersections.Select(tuple => GetDistance(tuple, wireTwoPath));

            var min = wireOneSteps.Zip(wireTwoSteps, (i, i1) => i + i1).Min();
            Console.WriteLine(min);

            return -1;
        }

        private int GetDistance((int, int) tuple, (int, int)[] wirePath)
        {
            var distance = 0;
            for (var k = 0; k < wirePath.Length-1; k++)
            {
                var maxX = Math.Max(wirePath[k].Item1, wirePath[k+1].Item1);
                var minX = Math.Min(wirePath[k].Item1, wirePath[k+1].Item1);

                var maxY = Math.Max(wirePath[k].Item2, wirePath[k+1].Item2);
                var minY = Math.Min(wirePath[k].Item2, wirePath[k+1].Item2);

                if ((tuple.Item1 <= maxX && tuple.Item1 >= minX) 
                    && (tuple.Item2 <= maxY && tuple.Item2 >= minY))
                {
                    distance += Math.Abs(wirePath[k].Item1 - tuple.Item1) +
                                Math.Abs(wirePath[k].Item2 - tuple.Item2);
                    break;
                }

                distance += Math.Abs(wirePath[k].Item1 - wirePath[k + 1].Item1) +
                            Math.Abs(wirePath[k].Item2 - wirePath[k + 1].Item2);
            }

            return distance;
        }

        private bool IsVertical((int, int) a, (int, int) b)
        {
            return a.Item1 == b.Item1;
        }

        private (int, int)[] ConstructPath((int, int)[] wireOneDirections)
        {
            var path = new List<(int, int)> {((rows / 2), (rows / 2))};
            for (var k = 0; k < wireOneDirections.Length; k++)
            {
                path.Add((path[k].Item1 + wireOneDirections[k].Item1, path[k].Item2 + wireOneDirections[k].Item2));
            }

            return path.ToArray();
        }

        private (int, int) ResolveDirection(string direction)
        {
            var magnitude = int.Parse(direction.Substring(1));
        ...
        private (int, int)[] ExtractDirections(string wire)
        {
            return wire
                .Split(",")
                .Select(ResolveDirection)
                .ToArray();
        }
    }
}